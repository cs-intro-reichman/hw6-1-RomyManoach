...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		Color[][] pic = new Color[image.length][image[0].length];
		
		for (int i = 0; i < image.length; i++){
			int index = 0;
			for (int j = image[0].length -1; j >= 0; j--){
				pic[i][index] = image[i][j];
				index++;
			}
		}
		return pic;
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for (int i = 0; i < image.length; i++){
			for (int j = 0; j < image[0].length; j++){
				pic[image.length -1 -i][j] = image[i][j];
			}
		}
		return pic;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < pic.length; i++){
			for (int j = 0; j < pic[0].length; j++){
				pic[i][j] = image[i * image.length / height][j * image[0].length / width];
			}
		}
		return pic;
	}
	...
	public static Color[][] blend(Color[][] image1, Color[][] image2, double alpha) {
		Color[][] pic = new Color [image1.length][image1[0].length];
		for(int i = 0; i < image1.length; i++){
			for (int j = 0; j < image1[0].length; j++){
				pic[i][j] = blend(image1[i][j], image2[i][j], alpha);
			}
		}
		return pic;
	}
